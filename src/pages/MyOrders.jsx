...
export default function MyOrders() {
  ...
  const orders = JSON.parse(localStorage.getItem('sh_orders') || '[]').filter(o => o.userId === user.id)
      ...
      <div style={{ padding: '40px 60px', minHeight: '70vh' }}>
        ...
        {orders.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {orders.map(order => (
              <div key={order.id} style={{ background: '#fff', border: '1px solid #e0c9a6', borderRadius: 14, padding: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    <p style={{ color: '#8b4513', fontWeight: 700, marginBottom: 4 }}>{order.id}</p>
